...
def MosflmRefineCell(DriverType=None, indxr_print=True):
    ...
    class MosflmRefineCellWrapper(CCP4DriverInstance.__class__):
        ...
        def set_ignore_cell_refinement_failure(self, ignore_cell_refinement_failure):
        ...
        def run(self):
            ...
            assert (
                self._input_mat_file is not None and self._output_mat_file is not None
            )
            ...
            self.input("postref multi segments %d repeat 10" % len(self._images))

            self.input("postref maxresidual 5.0")

            genfile = os.path.join(
                os.environ["CCP4_SCR"], "%d_mosflm.gen" % self.get_xpid()
            )
            ...
            for i in range(len(output)):
                ...
                if "Rms positional error (mm) as a function of" in o and True:
                    ...
                    while output[j].split():
                        if "Image" in output[j]:
                            for image in map(
                                int, output[j].replace("Image", "").split()
                            ):
                                images.append(image)
                        else:
                            ...
                            try:
                                ...
                                Debug.write(
                                    "Error parsing %s as floats" % output[j][12:]
                                )
                ...
                if (
                    "is greater than the maximum allowed" in o
                ...
                if "INACCURATE CELL PARAMETERS" in o:
                    ...
                    sd_record = (
                        output[i + 5].replace("A", " ").replace(",", " ").split()
                    )
                    sds = map(float, [sd_record[j] for j in range(1, 12, 2)])

                    Debug.write("Standard deviations:")
                    Debug.write(
                        "A     %4.2f  B     %4.2f  C     %4.2f" % (tuple(sds[:3]))
                    )
                    Debug.write(
                        "Alpha %4.2f  Beta  %4.2f  Gamma %4.2f" % (tuple(sds[3:6]))
                    )

                    Debug.write("In cell refinement, the following cell parameters")
