...
class MosflmIndexer(IndexerSingleSweep):
    ...
    def _index(self):
        ...
        if self._indxr_input_lattice is not None:
            spacegroup_number = lattice_to_spacegroup(self._indxr_input_lattice)
        ...
        if not self._mosflm_autoindex_thresh:
            ...
            try:
                ...
                for i in _images:
                    ...
                    Debug.write("Autoindex threshold for image %d: %d" % (i, thresh))
        ...
        if min(list(indxr_cell)) < 10.0 and indxr_cell[2] / indxr_cell[0] > 6:
        ...
        try:
            ...
            if self._indxr_user_input_lattice:
                ...
                for k in self._indxr_other_lattice_cell.keys():
                    if (
                        lattice_to_spacegroup_dict[k]
                        > lattice_to_spacegroup_dict[self._indxr_input_lattice]
                    ):
        ...
        if len(mosaic_spreads) == 0:
            ...
            phi_width = self.get_phi_width()
            Chatter.write("Mosaic estimation failed, so guessing at %4.2f" % phi_width)
        ...
        self._indxr_payload["mosflm_orientation_matrix"] = open(
            os.path.join(self.get_working_directory(), "xiaindex.mat"), "r"
        ).readlines()
