#!/usr/bin/env python
# LabelitDialsIndexer.py
#   Copyright (C) 2013 Diamond Light Source, Graeme Winter, Richard Gildea
#
#   This code is distributed under the BSD license, a copy of which is
#   included in the root directory of this package.
#
# An implementation of indexing which should work correctly with XDS but will 
# actually use Labelit for indexing and Dials for integration.
...
class LabelitDialsIndexer(FrameProcessor,
                          Indexer):
  '''An implementation of the Indexer interface using Labelit and Dials.'''
  ...
  def Refine(self):
    ...
    refine = Refine(params=PhilIndex.params.dials.refine)
    refine.set_working_directory(self.get_working_directory())
    refine.setup_from_image(self.get_image_name(
        self._indxr_images[0][0]))
  ...
  def AssignIndices(self):
    from Wrappers.Dials.AssignIndices import AssignIndices
    assignindices = AssignIndices(params=PhilIndex.params.dials.assign_indices)
    assignindices.set_working_directory(self.get_working_directory())
    assignindices.setup_from_image(self.get_image_name(
        self._indxr_images[0][0]))

    auto_logfiler(assignindices, 'ASSIGNINDICES')

    return assignindices


  def Import(self):
    from Wrappers.Dials.Import import Import
    importer = Import(params=PhilIndex.params.dials.import)
    importer.set_working_directory(self.get_working_directory())
    importer.setup_from_image(self.get_image_name(
        self._indxr_images[0][0]))

    auto_logfiler(importer, 'IMPORT')
