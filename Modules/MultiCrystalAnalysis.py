...
class multi_crystal_analysis(object):
    def __init__(
        ...
        for k in self.intensities.keys():
            ...
            self.individual_merged_intensities[k] = (
                self.intensities[k].merge_equivalents().array()
            )
    ...
    def relative_anomalous_cc(self):
        if self.unmerged_intensities.anomalous_flag():
            ...
            for i_wedge in self.individual_merged_intensities.keys():
                ma_i = self.individual_merged_intensities[i_wedge].resolution_filter(
                    d_min=d_min
                )
                anom_i = ma_i.anomalous_differences()
                anom_cc = anom_i.correlation(
                    full_set_anom_diffs, assert_is_similar_symmetry=False
                ).coefficient()
                racc.append(anom_cc)
            return racc
    ...
    def compute_correlation_coefficient_matrix(self):
        ...
        for i_wedge in self.individual_merged_intensities.keys():
            for j_wedge in self.individual_merged_intensities.keys():
                ...
                ma_j = self.individual_merged_intensities[j_wedge].resolution_filter(
                    d_min=d_min
                )
    ...
    def plot_cc_matrix(self, correlation_matrix, linkage_matrix, labels=None):
        ...
        y2_dict = scipy_dendrogram_to_plotly_json(
            ...
            xtitle="Individual datasets",
            ytitle="Ward distance",
        )  # above heatmap
    ...
    def write_output(self):
        ...
        with open("intensity_clustering.txt", "wb") as f:
            ...
            f.write(table_utils.format(rows, has_header=True, prefix="|", postfix="|"))
