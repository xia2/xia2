...
def writeLoggraphFolder(html, program, n):
    ...
    for i in range(0, ntables):
        ...
        html.write(
            '<p id="' + str(name) + "_loggraph_title_" + str(i) + '" '
            'style="font-weight:' + str(fontWeight) + '">'
        )
        html.write(
            '<a href="javascript://" onclick="pick_loggraph(\''
            + str(name)
            + "',"
            + str(i)
            + ","
    ...
    for i in range(0, ntables):
        ...
        if not tbl.parse_error():
            if tbl.ngraphs() > 0:
                ...
                html.write(
                    '<div class=applet><applet archive="' + jloggraph.archive() + '"\n'
                )
...
def writeFragmentFolder(html, logfile, fragment, n):
    ...
    html.write(
        "      onclick=\"open_fragment('" + str(n) + "')\">[View fragment]</a>\n"
    )
    html.write("   </p>\n")
    html.write("</div>\n\n")
    ...
    html.write('      <a href="javascript://"\n')
    html.write(
        "         onclick=\"close_fragment('" + str(n) + "')\">[Hide fragment]</a>\n"
    )
    html.write("   </p>\n")
...
def writeAdvancedLogfileFolder(html, logfile, program, n):
        ...
        if found_summary:
            ...
            if logfile.summary(i).start() > prog_end:
                ...
                writeLogfileNonSummary(
                    html, name, smartie.retrieve(logfilen, curr_line, prog_end)
                )
            ...
            writeLogfileNonSummary(
                html,
                name,
                smartie.retrieve(logfilen, curr_line, logfile.summary(i).start() - 1),
            )
            ...
            if (
                logfile.summary(i).start() <= prog_end
                and logfile.summary(i).end() >= prog_end
            ):
    ...
    if found_summary and logfile.summary(i).end() < prog_end:
        writeLogfileNonSummary(
            html,
            name,
            smartie.retrieve(logfilen, logfile.summary(i).end() + 1, prog_end),
        )

    # If there were no summaries then just dump the log with no processing
    # Mark it as summary
    if not found_summary:
        writeLogfileSummary(html, program.retrieve())
...
def writeLogfileControls(html, name):
    ...
    html.write('   <a href="javascript://"\n')
    html.write(
        "      onclick=\"open_summary_logfile('"
        + str(name)
        + "')\">[Show logfile summary]</a>\n"
    )
    html.write('   <a href="javascript://"\n')
    html.write(
        "      onclick=\"open_full_logfile('"
        + str(name)
        + "')\">[Show full logfile]</a>\n"
    )
    html.write("   </p>\n")
    ...
    html.write('      <a href="javascript://"\n')
    html.write(
        "         onclick=\"show_full_logfile('"
        + str(name)
        + "')\">[Show full logfile]</a>\n"
    )
    html.write('      <a href="javascript://"\n')
    html.write(
        "         onclick=\"close_logfile('" + str(name) + "')\">[Hide logfile]</a>\n"
    )
    html.write("     </p>\n")
    html.write("   </div>\n")
...
def writeLogfileNonSummary(html, name, text):
    ...
    html.write('  <div class="' + str(name) + '_inner_control inner_control">\n')
    html.write(
        '     <a href="javascript://" onclick="show_only_summary(\''
        + str(name)
        + "')\">[Show summary only]</a>\n"
    )
    html.write("  </div>\n")
...
def writeBanner(html, name, program, anchor):
    ...
    try:
        html.write(
            '<span class="progvers">Version ' + str(program.version) + "</span>\n"
        )
    except AttributeError:
        pass
    # Run time/date
    ...
    try:
        html.write(
            '<span class="progterm">Finished with: '
...
class extendedpatternmatch(smartie.patternmatch):
    ...
    def islibcheckbanner(self, text):
        ...
        #
        banner = self.compile(
            "islibcheckbanner", r"  --- LIBCHECK --- /Vers ([0-9.]+); ([0-9.]+)/\n"
        ).search(text)
        result = dict()
        if banner:
            result["banner_text"] = banner.group(0)
...
def processMatthews_Coef(program):
    ...
    for line in logtext.split("\n"):
        ...
        if result:
            ...
            table.settitle("Matthews_Coeff mol weight " + str(program.mol_weight))
...
def summariseGeneric(html, program):
    ...
    if warnings:
        ...
        for warning in warnings:
            html.write('<li><span class="Warning">' + str(warning) + "</span></li>\n")
        html.write("</ul>")
        html.write("</div>\n")
...
def summariseMatthews_Coef(html, program):
    ...
    if program.has_attribute("mol_weight"):
        html.write("<p>Molecular weight " + str(program.mol_weight) + "</p>")
...
def summarisePhaser(html, program):
    ...
    if program.has_attribute("phaser_module"):
        html.write(
            "<p><b>Phaser module</b>: " + str(program.phaser_module).title() + "</p>"
        )
    # Write a list of warnings
    summariseGeneric(html, program)
...
def polarrfn_plot(pltfile, imgbase=""):
    ...
    try:
        ...
        os.popen(
            "pltdev -xp 0.7 -yp 0.7 -dev ps -o " + str(psfile) + " " + str(pltfile)
        )
    except Exception:
...
<!-- Start of logfile -->\n"""
    ...
    if log.nprograms() > 1:
        ...
        html.write(
            "<p>The logfile is composed of output "
        ...
        for i in range(0, log.nfragments()):
            ...
            if fragment.isprogram():
                ...
                if program.has_attribute("termination_message"):
                    termination = "Finished with: " + str(program.termination_message)
                else:
                    termination = ""
    ...
    if log.nfragments() > 0:
        for i in range(0, log.nfragments()):
            ...
            if fragment.isprogram():
                ...
                ccp4i_info = fragment
                html.write(
                    '<p><span class="ccp4i">CCP4i:</span> <em>'
    ...
    html.write("<hr />\n")
    html.write(
        "<p><em>Generated for you by baubles "
        + str(__version__)
        + " on "
        + str(date)
        + "</em></p>\n"
