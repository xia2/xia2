...
"""
    ...
    def _write_mmcif_output(self):
        ...
        for dname in dnames:
            ...
            # get the block without reflection data to add to the mtz appendix
            block = mmCIF.get_block(blockname)
            reduced_block = copy.deepcopy(block)
            for k in list(reduced_block.keys()):
                if k.startswith("_diffrn_refl") or k.startswith(
