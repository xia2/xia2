...
two_images_indexing = {"beam_centre": (94.33, 94.58), "mosaicity": 0.4}
...
def test_index_two_images_with_mosflm(ccp4, dials_data, run_in_tmpdir):
    ...
    assert approx_equal(
        indexer.get_refined_beam_centre(), two_images_indexing["beam_centre"]
    )
    assert approx_equal(
...
def test_indexing_multiple_images_with_mosflm(
    regression_test, ccp4, dials_data, run_in_tmpdir
):
    templ, directory = get_template_and_directory(dials_data)
    ...
    assert approx_equal(
        indexer.get_refined_unit_cell(), (78.2082, 78.2082, 78.2082, 90.0, 90.0, 90.0)
    )
...
SYMM I23       \n"""
    ...
    for cycle in background_residual:
        for frame in background_residual[cycle]:
            assert background_residual[cycle][frame] == pytest.approx(
                ref_residual[cycle][frame], abs=0.1
            )
    ...
    for cycle in rms_values:
        for frame, value in enumerate(rms_values[cycle]):
            assert value == pytest.approx(ref_values[cycle][frame], abs=0.05)
