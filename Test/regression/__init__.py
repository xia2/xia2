...
def check_result(test_name, result, tmpdir, ccp4, xds=None, expected_data_files=default_data_files):
  ccp4 = ccp4['version']
  ...
  if result['exitcode']:
    return False, "xia2 terminated with non-zero exit code (%d)" % result['exitcode']
  ...
  for f in os.listdir(expected_result_dir):
    if f.startswith('result.%s' % test_name) and os.path.isfile(os.path.join(expected_result_dir, f)):
      ...
      if candidate_version:
        candidate_version = [int(v) if v else 0 for v in candidate_version.group(1,2,3,5)] + [candidate_version.group(7)]
        ...
        if xdsv and (xdsv < xdsrev): continue
        # ensure file is not made for a more specific platform
        if candidate_platform and candidate_platform != 'Linux' and candidate_platform != system: continue
        if expected_result_file is not None and expected_result_file_version is not None:
          ...
          if cmaj == major and cmin > minor: continue
          if cmaj == major and cmin == minor and crev > revision: continue
          if xds and xdsv > xdsrev: continue
          if plat == system and candidate_platform != system: continue
