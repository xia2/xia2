...
def generated_exp(n=1, space_group="P 2", assign_ids=False, id_=None):
  ...
  exp_dict = {"__id__" : "crystal", "real_space_a": [15.0, 0.0, 0.0],
              "real_space_b": [0.0, 10.0, 0.0], "real_space_c": [0.0, 0.0, 20.0],
              "space_group_hall_symbol": space_group}
  crystal = Crystal.from_dict(exp_dict)
  scan = Scan(image_range=[0, 90], oscillation=[0.0, 1.0])
  beam = Beam(s0=(0.0, 0.0, 1.01))
  if assign_ids:
    experiments.append(Experiment(identifier="0", beam=beam, scan=scan, crystal=crystal))
  elif id_:
    experiments.append(Experiment(identifier=str(id_), beam=beam, scan=scan, crystal=crystal))
  else:
    experiments.append(Experiment(beam=beam, scan=scan, crystal=crystal))
  if n > 1:
    for i in range(1, n):
      if assign_ids:
        experiments.append(Experiment(identifier=str(i), beam=beam, scan=scan, crystal=crystal))
      else:
        experiments.append(Experiment(beam=beam, scan=scan, crystal=crystal))
  return experiments
...
def generate_reflections_in_sg(space_group, id_=0, assign_id=False):
  ...
  intensities = generate_intensities(
    cs, d_min=2.0).generate_bijvoet_mates().set_observation_type_xray_intensity()
  ...
  reflections.set_flags(flex.bool(reflections.size(), True), reflections.flags.integrated)
  return reflections
...
def test_assign_identifiers(helper):
  ...
  assigned_exp = load.experiment_list(
...
def test_split_experiments(number_of_experiments, helper):
  ...
  dump.experiment_list(generated_exp(number_of_experiments, assign_ids=True), exp_path)
  reflections = flex.reflection_table()
...
class simple_refiner(object):
  ...
  def __init__(self, refiner_lattices):
    self.refiner_lattices = refiner_lattices #first one should be 'best' one used in refinement
...
test_data = [
  ...
  (['mP', 'aP', 'oP'], ['aP'], ('aP', False, True)), #symmetry is lower than from refiner
  (['mP', 'aP', 'oP'], ['tP', 'mP'], ('mP', True, False)), #symmetry finds higher than refiner
  (['mP', 'aP', 'oP'], ['tP', 'aP'], ('aP', True, True))] #symmetry finds higher than refiner,
...
test_lattices = [
  ...
  (['aP'], ('P 1', False, False, False, True))]#symmetry finds higher than refiner - can occur
