...
def B(a, b, c, alpha, beta, gamma):
    ...
    if (
        (math.fabs(la - a) / la) > 0.001
        or (math.fabs(lb - b) / lb) > 0.001
        or (math.fabs(lc - c) / lc) > 0.001
    ):
...
def wilson(nu, nm, isigma):
    ...
    for c in range(nu):
        ...
        result = []
        for m in range(nm):
            result.append((random.gauss(imean, imean / isigma), imean / isigma))
...
def cc(a_, b_):
    ...
    return (ab - a * b) / math.sqrt((a2 - a * a) * (b2 - b * b))
...
def xds_integrate_header_read(xds_hkl):
    ...
    beam = (wavelength * beam[0], wavelength * beam[1], wavelength * beam[2])
...
def xds_integrate_hkl_to_list(xds_hkl):
    ...
    cell, pixel, origin, distance, wavelength = xds_integrate_header_read(xds_hkl)
...
def outlier(sisigma):
    ...
    for j in range(500):
        ...
        fout.write(
            "%f %f %f\n"
            % (0.004 * (j + 1), result[0.004 * (j + 1)][0], result[0.004 * (j + 1)][1])
        )
...
def digest(bins, isigma_limit=1.0):
    ...
    Debug.write(
        "Selected resolution range: %.2f to %.2f for Wilson fit"
