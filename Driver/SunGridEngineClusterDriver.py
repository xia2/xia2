...
class SunGridEngineClusterDriver(DefaultClusterDriver):
    ...
    file channel opened when the process has finished..."""
        ...
        pipe = subprocess.Popen(
            ["qsub", "-V", "-cwd", "%s.sh" % self._script_name],
    ...
    def cleanup(self):
        ...
        sge_stdout = os.path.join(
            self._working_directory, "%s.sh.o%s" % (self._script_name, self._job_id)
        )

        sge_stderr = os.path.join(
            self._working_directory, "%s.sh.e%s" % (self._script_name, self._job_id)
        )
